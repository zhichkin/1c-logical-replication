...
Функция ВычислитьИдентификаторДляНабораЗаписей(Источник) Экспорт
	...
	Для Каждого ЭлементОтбора Из Источник.Отбор Цикл
		...
		MD5.Добавить(ЗначениеКакСтрокаXML(Значение));
		
	КонецЦикла;
	
	Хеш = СтрЗаменить(MD5.ХешСумма, " ", "");
	
	Результат = Сред(Хеш,  1,  8)
	    + "-" + Сред(Хеш,  9,  4)
		+ "-" + Сред(Хеш, 13,  4)
		+ "-" + Сред(Хеш, 17,  4)
		+ "-" + Сред(Хеш, 21, 12);
	...
	Попытка
		Идентификатор = Новый УникальныйИдентификатор(Результат);
	Исключение
...
Функция ЗначениеКакСтрокаXML(Значение)
	...
	ИначеЕсли ИмяТипаЗначения = "ВидДвиженияНакопления" Тогда
		...
		Возврат ?(Значение = ВидДвиженияНакопления.Приход, "Приход", "Расход");
